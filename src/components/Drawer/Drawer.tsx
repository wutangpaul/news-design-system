...
/**
 * WAI-ARIA APG focus trap, shared in shape with Modal's: moves focus into
 * `containerRef` on activation, cycles Tab/Shift+Tab within it, invokes
 * `onClose` on Escape, and restores focus to the previously-focused element
 * on deactivation.
 */
...
const drawerVariants = cva(
  "fixed flex flex-col bg-surface-raised text-text-primary shadow-xl transition-transform duration-base ease-standard",
  {
    variants: {
      side: {
        left: "inset-y-0 left-0 h-full w-full max-w-sm",
        right: "inset-y-0 right-0 h-full w-full max-w-sm",
        top: "inset-x-0 top-0 max-h-[80vh] w-full",
...
export interface DrawerProps extends Omit<HTMLAttributes<HTMLDivElement>, "title"> {
  /** Whether the drawer is open. Renders nothing in the DOM while closed. */
  open: boolean;
  /** Invoked on Escape, backdrop click, or the built-in close button. */
  onClose: () => void;
  /** Heading rendered in the drawer header and referenced by `aria-labelledby`. */
  title: ReactNode;
  /** Optional supporting copy referenced by `aria-describedby`. */
  description?: ReactNode;
  /** Edge of the viewport the drawer slides in from. @default "right" */
  side?: DrawerSide;
...
/**
 * Edge-anchored panel sharing Modal's dialog semantics and focus-management
 * behavior (focus trap, Escape-to-close, backdrop-click-to-close, body
 * scroll lock, return-focus-on-close). Typically used for mobile navigation
 * or filter panels.
 */
export const Drawer = forwardRef<HTMLDivElement, DrawerProps>(function Drawer(
  ...
  return createPortal(
    ...
    <div className="fixed inset-0 z-drawer bg-ink-950/60" onMouseDown={handleBackdropMouseDown}>
      <div
        ref={(node) => {
        ...
        className={cn(
          drawerVariants({ side }),
          entered ? "translate-x-0 translate-y-0" : closedTransform[side],
          className,
        )}
        {...rest}
      >
